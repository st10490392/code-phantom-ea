...
Status: **decision-ready design proposal; not a frozen executable hypothesis**.
Prepared against repository checkpoint `0836cf73af6b6cf98e3e73fc034da52a735cd820`.
This document develops, rather than replaces, the [248-line temporal design audit](CP-002-temporal-hypothesis-design-audit.md). Its primitive inventory and reconstruction limitations remain applicable. No implementation or experiment is authorized by this document.
...
Exactly one status per original audit ID. Scope is the proposed initial R campaign and its explicitly conditional F/W variants; other families have additional gates in section 19. A compound decision stays OPEN if any necessary part remains unresolved. DEFERRED/EXCLUDED mean no operative rule in the initial campaign, not a hidden positive predicate.

Disposition totals: **16 RESOLVED, 38 OPEN, 2 CAMPAIGN_VARIABLE, 3 DEFERRED, 1 EXCLUDED = 60**. Deferred D-IDs are D26/D29 (IFVG) and D36 (OTE); D57 excludes advanced management/execution-cost modeling. Other deferred model families and concepts are listed separately rather than assigned new meanings to existing D-IDs.
...
| D02 | RESOLVED | Use frozen Revision 2 primitive behavior as the proposed R detector contract, with reference-engine semantics; retain swing window 2, liquidity/equilibrium tolerance 0, reclaim window 1, displacement lookback 5, range multiple 1.5, body ratio 0.6. Do not inherit CP-001's simultaneous candidate gates, legacy sequence helper or automatic historical IFVG preference. D17 must accept its MSS interpretation before freezing. | Existing settings inherited without a search; I, P. This resolves the proposed primitive identity, not D17's fit to manual intent. |
...
| D06 | OPEN | Which stages require H4 direction, and what is “sideways”? Decide H4-aligned M15 reversal versus H4-turn model; define neutral/unavailable behavior and whether repository bias is an acceptable trend proxy. | Manual regime intent is broader than available bias; R, P. Need researcher predicates, not a new invented range detector. |
| D07 | OPEN | Must R's MSS direction match H4, or can it oppose H4 pending a defined transition? Choose H4 reference for the selected branch. Only a completed reference available at the evaluation may be used. | Reversal scope ambiguous; R, P, E. |
| D08 | OPEN | Continuous alignment, entry recheck or latched context? Define changed-and-restored cases and intermediate H4 events across M15 gaps. | A latest pointer cannot prove continuity; R, P. |
| D09 | OPEN | For R, are wick/reclaim sweeps mandatory, or is important structural context without a raid allowed? Specify eligible set/branch. Accepted break cannot count as a reversal sweep; pending breach is not confirmed evidence. | “Where applicable” leaves a real distinction; R, P. Need explicit researcher selection. |
...
| D12 | OPEN | Is liquidity confirmation strictly before MSS or may they share completion? Approve `l<m` or `l≤m`, and clarify context-only branch if D09 permits it. | Co-availability does not reveal intrabar sequence; R, E. |
...
| D14 | OPEN | Specify maximum liquidity age, clock, inclusive boundary and checked stages, or explicitly accept no age cap. Need pre-outcome rationale. | No numerical recency supplied; R, E. |
...
| D17 | OPEN | Accept repository opposing-BOS MSS (including its structural-class policy) as the exact V1 proxy, or defer R until a recent-HL/LH contract is supplied? No automatic CHoCH alias or BOS substitute. | Manual recent HL/LH can differ from active external level; R, P, S. |
| D18 | OPEN | First/latest/repeated MSS association; new setup versus replacement. Specify whether repeated evidence can refresh any deadline. | Existing current-trigger gate gives no temporal ownership; P, E. |
...
| D22 | OPEN | Choose first versus later/repeated qualifying displacement and handling of opposite/missing evidence; approve or reject the first-match proposal in section 13. | Needed to make W deterministic; R, E. |
...
| D24 | OPEN | Select among multiple associated ordinary FVGs with explicit source/availability ordering and ties. Do not inherit latest historical IFVG/FVG pointer selection. | Associated array intent requires ownership; R, P, E. |
...
| D30 | OPEN | Maximum ordinary FVG age from creation and checking stages, or explicit no-cap answer. IFVG age origins are deferred. | No age supplied; R, E. |
| D31 | OPEN | Freeze one selected array, replace under a declared rule, or retain several while waiting? Define identity/deadline consequences. | Latest selection must not silently rewrite a setup; R, E. |
| D32 | OPEN | Exact structural leg anchor IDs and availability; frozen versus updating range; missing/invalid/reset handling. Need a causal anchor algorithm, not only midpoint arithmetic. | User's relevant dealing leg is not automatically current engine range; R, P. |
| D33 | OPEN | Exact entry area: entire associated FVG, named sublevel, range-zone intersection or another rigorous existing geometry? Select one; do not search depths. | Touch needs a concrete interval/level; R, P. |
| D34 | OPEN | Wick-range overlap, body penetration or close condition; depth, inclusive boundaries and directional approach? Supply exact OHLC predicate and known-at time. | Alternatives imply different observations/fills; R, E. |
| D35 | OPEN | Must price depart before re-entry? Define initial-inside, gap-through and repeated-touch behavior and departure availability. | Later candle alone need not mean a genuine retracement; R, E. |
...
| D37 | OPEN | Specify entry-stage discount for bullish/premium for bearish, equilibrium equality, and whether price means entry reference or touched level; link to D32. Never test the MSS candle as the opposite-zone retracement. | Temporal intent clear, exact predicate unresolved; R, P. |
...
| D39 | OPEN | Setup/stage lifetime, origin, clock, inclusive deadline and reset rules; explicit no-limit option needs a resource plan. No lifetime inferred from 96-bar outcome horizon. | Intent supplies no age value; R, E. |
...
| D41 | OPEN | H4 change/neutrality: cancel, suspend or entry-only rejection? Define restoration and missing intermediate context. | Linked to D06–D08; R, P. |
...
| D46 | OPEN | Maximum active setups per instrument/direction and capacity action, or explicitly unbounded semantics with disk/resource plan. Instruments remain independent for initial replication. | Old machine requires bounds without silently discarding valid setups; E, R. |
| D47 | OPEN | Competing eligible setups sharing event/array/entry time: retain all or select by exact priority? Define tie-breaking and cross-family policy if later combined. | Changes the observation population; E, R. |
| D48 | OPEN | Duplicate key/lifetime, event reuse, retries and consumed transition. Proposed immutable setup ID alone does not decide whether one event may seed another setup. | Deterministic replay and population control; E. |
...
| D50 | CAMPAIGN_VARIABLE | A: area reach without LTF confirmation. F: same parent area, followed by one frozen LTF confirmation on one declared M1 or M5 stream. Exact contact/fill D51 and F predicate/clock must be closed before either comparison is frozen; no A fallback after F failure. | Explicit pre-outcome execution alternatives; R. Additional F choices are OPEN, not a parameter grid. |
| D51 | OPEN | Choose actual level-touch versus close-after-contact A; F confirmation/entry price/time; gap-through, unavailable price and risk-before-entry rules. Need synthetic examples proving no use of later enclosing-bar information. | Fill and availability cannot be hand-waved; R, I, E. |
...
| D55 | OPEN | Retain 96 subsequent M15 bars for completion-close baseline, excluding entry candle, with censoring at authorized tail. Freeze a comparable exact horizon/counting rule for intrabar A/F and remaining entry interval before those cells are admitted. | Inheritance preferred but finer entry clocks require an explicit extension; I, R, E. |
| D56 | OPEN | Retain conservative stop-first ambiguity for post-entry OHLC ties and distinguish full-window excursions from stopped paths. Specify intrabar entry-bar/gap order and finer-stream label semantics; no pre-entry extremes included. | Existing conventions cover close-entry, not every proposed fill; I, P, E. |
...
## 19. Remaining researcher decisions

All OPEN IDs in section 14 require answers with rationale, provenance, exact predicate/units and dependencies. Resolve them in this order to avoid choosing numbers before the model's meaning:

1. **Model meaning:** D06–D10 and D17: H4 relationship/regime, required liquidity and structural proxy. Decide whether initial R is a pullback reversal aligned with H4 or a different model. Do not freeze a detector named MSS without accepting what it actually breaks.
2. **Parent association:** D12, D15/D16/D18, D22, D24/D25/D27/D28/D31: ordering, competing events, FVG parent and lifecycle ownership.
3. **Entry geometry:** D32–D35/D37 and D51: leg, area, departure and actual price/time convention. F additionally needs one LTF, exact confirmation, arming, deadline and failed-confirmation/retry rules.
4. **Lifetime/cancellation/population:** D14/D20/D30/D39–D48: ages, clocks, terminal guards, overlap and duplicates. No numerical bound is suggested by existing results or inherited outcome horizon.
5. **Coverage/endpoints:** D03/D45/D55/D56: start/reset, missing data, entry-interval ordering and comparable labels.
6. **Campaign inclusion:** accept, reduce or defer the proposed cells before observing outcomes; decide W and F readiness without expanding to a grid. D19/D50 remain CAMPAIGN_VARIABLE, not resolved by a favorable run.

Additional scope questions outside the original D-register:

| Key | Researcher/source decision required | Initial campaign treatment |
|---|---|---|
| C01–C04 | Exact trend/sideways classifier; causal HL→HH/LH→LL anchor algorithm; frozen/updated leg and pullback termination; array overlap/optional OTE and cancellation. | Continuation deferred until separate specification is complete. |
| S01–S04 | Named session windows/zones/trading dates; range completeness/evolving versus final levels; Asia/London/NY eligibility relationships; day/week calendars and reset/target rules. | Session filters/targets deferred. |
| SB01 | Verify original Silver Bullet passages and all section 10 timing/sequence questions. | SOURCE_VERIFICATION_REQUIRED; no runnable cell. |
| F01–F04 | One M1/M5 stream and aggregation; exact LTF confirmation; arming/deadline/retry; entry and partial-M15 outcome alignment. | Blocks R-F-S; also covered by D50/D51/D55/D56. |
| W01–W02 | OHLC range/sequence definitions; separate volume methodology and data basis. | Wyckoff descriptive design only; no filter. |
| P01 | Primary-source concepts/interpretations in section 5, especially OB/breaker, IFVG, OTE, CRT, AMD/Judas and SMT. | Unverified or undefined concepts remain deferred; no invented detectors. |

An explicit exclusion or “no limit” is a valid answer when its consequences are documented. Engineering proposals in RESOLVED rows still need acceptance as part of the final decision sheet. This task does not require the researcher to answer now and does not make unresolved choices on their behalf.

## 20. Proposed freeze checklist
...
## 21. Explicit non-claims and limitations
