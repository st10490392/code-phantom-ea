...
## CP-002 Semantic Closure V1 — Approved PRE-RESULT

This amendment supersedes conflicting Freeze V1 and pre-freeze proposals below. Decisions were supplied by the researcher before CP-002 market outcomes. Only design and synthetic implementation are authorized; semantic run-readiness is not permission to run historical research.

- **Initialization/continuity:** process each permitted dataset chronologically from its first real completed candle. No synthetic prehistory or fixed warm-up count. Eligibility begins only when required primitives exist. Calendar/year/reporting slices do not reset detector, setup or consumption state. An authorized contiguous partition boundary would preserve causal state; crossing into a locked partition is prohibited. End the run at the allowed boundary, censor incomplete outcomes there, and never read later data to complete labels. A new unrelated dataset has a new namespace and cannot inherit fabricated state.
- **Coverage:** observed-bar indices count real candles. Preserve gaps without interpolation. Explicitly evidenced market closures preserve active setups and detector state. Unexplained/material missing coverage produces DATA_COVERAGE_FAILURE for affected paths; absence of evidence is not evidence of no event. A failed detector stream cannot silently resume with assumed state. Coverage evidence must identify the actual missing interval and source reference; elapsed time alone cannot certify a weekend/closure. M5 gaps affect F, not independent A experiments.
- **Liquidity:** existing confirmed M15 structural swing pools, with inherited zero-tolerance exact grouping and internal/external classification, only. SAME_CANDLE_SWEEP: bullish low < low-side level and close > level; bearish high > high-side level and close < level. Equality is TOUCH. A trade-through closing beyond is a break observation, not a reversal sweep or later reclaim. No delayed-reclaim window is introduced. Retain every independently qualifying unused parent for the MSS. Consumed event generations cannot seed another sweep setup; regrouping/new confirmed membership gets a new identity. Structural reversal has no required liquidity parent. Observable geometry makes no claim of institutional intent.
- **Strict ordinary FVG ownership (correction):** C1/C2/C3 are adjacent actual completed M15 candles; C2 must be the exact qualifying same-direction displacement. Bullish C3.low > C1.high gives [C1.high,C3.low]; bearish C3.high < C1.low gives [C3.high,C1.low]. Equality gives no FVG. Availability is C3 completion, never C2. **One single-candle displacement owns at most one same-timeframe ordinary V1 FVG.** Multiple independent liquidity-parent paths may reference that same single FVG; they are not multiple different FVG children. Earlier generic sibling fixtures remain archived capabilities, not V1 eligibility.
- **Lifecycle:** log CREATED/AVAILABLE/UNTOUCHED and subsequent CONTACTED/PARTIALLY_INTERACTED, FULLY_TRAVERSED and CLOSE_THROUGH as observations. Percentage mitigation is not a strategy invalidation rule. First later inclusive range intersection is contact; C3 cannot be its own retracement. Future facts cannot erase earlier observations.
- **Cancellation/invalidation:** before entry, opposite confirmed M15 MSS cancels with CANCELLED_OPPOSITE_MSS. Freeze the protected M15 low/high at parent MSS as the separate pre-entry anchor. Missing/nonfinite/invalid protection prevents executability. Subsequent bullish low ≤ anchor or bearish high ≥ anchor structurally invalidates; never move this anchor to rescue a path. At entry, separately freeze the latest causally available protected M15 stop per inherited risk policy. Post-entry MSS does not rewrite entries.
- **Ambiguity:** contact and structural breach in the same unresolved OHLC interval produce AMBIGUOUS_CONTACT_INVALIDATION and no entry/loss label. Preserve all simultaneous causes (including opposite MSS) rather than deleting evidence. A normal first contact is not itself invalidation. A full gap-through without intersection is explicitly unfillable, not a fabricated fill. A contact whose approach has no approved reference-price rule fails closed with an explicit unsupported-approach reason.
- **A entry:** first later completed M15 intersection. Store the interaction interval, candidate_available_at=its completion and reference price separately. Open inside inclusive zone => open; otherwise normal bullish approach from above => upper boundary, bearish from below => lower boundary. No claim of executable intrabar/live fill. The stop snapshot is the completed M15 snapshot available when the candidate becomes known, not a retroactive earlier snapshot choice.
- **F entry:** contact first, then M5_CONFIRMATION_ARMED. First strictly subsequent same-direction completed M5 opposing-BOS proxy supplies reference close. No pre-contact or equal-completion confirmation, M1, wick substitute or A fallback. Material missing M5 coverage after arming fails the path. A production F gate may stay disabled until the coverage/scheduling implementation is complete; semantic closure alone is not implementation verification.
- **Outcomes:** separate label stage only. A: 96 subsequent completed M15 candles excluding the contact candle. F: first full M15 interval whose **start is strictly after** M5 confirmation (including when confirmation equals an M15 boundary), then 96 full observed M15 candles. Exclude partial/intersecting intervals entirely. Post-entry unresolved SL+2R coincidence => STOP_FIRST; neither within complete horizon => TIMEOUT; incomplete authorized tail => CENSORED when no earlier threshold resolves the race. Excursions are descriptive full-window observations. Pre-entry ambiguity remains no trade, not a loss.
- **Still unresolved:** D20 numerical W. Production uses S only. Delayed reclaim is deferred with no window value. No session/IFVG/OB/breaker/OTE/CRT/AMD/Judas/Wyckoff/SMT/supply-demand implementation. No CP-001 or Observer V1 changes, historical data access, market experiment, optimization or connectivity.

Implementation entrypoints and exact verification are recorded in `docs/CP-002-semantic-closure-v1.md`. The old `research.cp002` fixture-only reducer is retained for reproducibility and continues to refuse production configurations; the strict V1 path is separately versioned so generic fixture associations cannot enable production eligibility.

## CP-002 Freeze V1 — Approved Research Decisions (superseded where amended above)

These decisions were explicitly approved by the researcher **PRE-RESULT**, before CP-002 historical execution or outcome inspection. Approval is not empirical evidence. The Semantic Closure amendment above and updated section 14 are normative; this earlier freeze is retained for provenance. Sections 1–13 and 15–18 below preserve the earlier design/provenance discussion: conflicting proposals, optional M1, guessed selection policies and earlier OPEN questions there are superseded by this freeze, not alternative defaults. Section 19 lists the remaining blockers. Deferred models remain deferred.
...
Disposition totals: **52 RESOLVED, 1 OPEN, 2 CAMPAIGN_VARIABLE, 4 DEFERRED, 1 EXCLUDED = 60**. Deferred D-IDs are D26/D29 (IFVG), D32 (setup-specific leg) and D36 (OTE); D57 excludes advanced management/execution-cost modeling. Other deferred model families and concepts are listed separately rather than assigned new meanings to existing D-IDs.
...
| D03 | RESOLVED | Process a dataset chronologically with natural primitive readiness, no fixed warm-up or fabricated history. Preserve state across reporting/year boundaries. Do not cross locked partition boundaries; censor labels at authorized end, retaining causal continuity only within authorized inputs. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D09 | RESOLVED | Separate SWEEP_REVERSAL requiring SAME_CANDLE_SWEEP and STRUCTURAL_REVERSAL without sweep. Delayed reclaim is deferred; accepted/pending break never satisfies sweep eligibility. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
| D10 | RESOLVED | Confirmed M15 structural swing pools only; preserve internal/external metadata and existing zero-tolerance grouping. Both classes are in the universe; no extra importance/session/day/week filter. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D15 | RESOLVED | Preserve all independently qualifying unused directional sweep-event parents for each MSS as separate immutable paths. No later-outcome selection. Stable ID ordering affects serialization only. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
| D16 | RESOLVED | Consume a sweep-event generation once when it seeds an MSS path, including a subsequently failed path. New confirmed pool membership/generation has a distinct ID. A previously closing-beyond generation cannot later qualify as SAME_CANDLE_SWEEP; delayed reclaim remains deferred. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D24 | RESOLVED | Strict V1 has at most one ordinary FVG per exact single-candle C2 displacement. Independent parent paths can reference the same array. Generic multi-child fixture architecture does not authorize additional V1 arrays. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
| D25 | RESOLVED | FVG is exactly adjacent completed C1/C2/C3 with qualifying displacement at C2. Available only at C3 completion. No before-MSS/arbitrary later array attachment for S. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D27 | RESOLVED | Exact parent edge is C2 identity to its one C1/C2/C3 gap. Geometry is strict third-low > first-high (bullish), third-high < first-low (bearish), with mirrored boundaries. No equality gap. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
| D28 | RESOLVED | Observe first later inclusive contact, partial interaction, full traversal and close-through separately. No percentage-fill invalidation. Formation contact forbidden. Later lifecycle cannot erase earlier contact; ambiguity and gap guards are D43/D44. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D31 | RESOLVED | Freeze the single strict array identity and parent per V1 path. Distinct parent generations remain independent. No replacement by newer arrays or multi-array impulse extension in V1. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D40 | RESOLVED | Opposite confirmed M15 MSS cancels pre-entry paths with CANCELLED_OPPOSITE_MSS; post-entry events cannot delete or retroactively cancel entries. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D42 | RESOLVED | Freeze parent-MSS protected M15 low/high for pre-entry checks. Later low ≤ bullish anchor / high ≥ bearish anchor invalidates. No moving buffer/fallback. Missing/nonfinite/wrong-side anchor blocks executable setup. Entry stop remains separate latest available M15 reference. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
| D43 | RESOLVED | First contact/partial/full traversal is observational, not automatic invalidation. Full gap-through without interval overlap is unfillable. Structural breach or opposite MSS terminates pre-entry path; consumed liquidity/new arrays do not rewrite frozen parents. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
| D44 | RESOLVED | Same-interval contact plus frozen structural breach => AMBIGUOUS_CONTACT_INVALIDATION, no trade/loss. Record all fired causes. Opposite MSS pre-entry also blocks entry. Post-entry SL/TP tie uses STOP_FIRST independently. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
| D45 | RESOLVED | Preserve source gaps; never interpolate. Explicitly evidenced normal closure preserves state. Unexplained/material missing coverage affecting required transition => DATA_COVERAGE_FAILURE on affected path/stream, no silent reset or inferred no-event. Coverage provenance distinguishes these cases. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D50 | CAMPAIGN_VARIABLE | A: later strict-FVG contact with open-inside/expected-side boundary reference. F: subsequent M5 MSS proxy, reference confirmation close; no M1 or A fallback. Timing and outcome semantics are now resolved, but production F stays disabled until joint stream coverage/scheduling is implemented and verified. | R, E; pre-result alternatives, distinct from implementation readiness. |
| D51 | RESOLVED | A uses open if inside zone, otherwise expected-side near boundary on later overlapping M15 interval; candidate available at completion. No gap-through or unsupported-approach fill. F uses first subsequent same-direction M5 proxy confirmation close; no pre-contact/equal-time confirmation or A fallback. Record timing separately from reference price. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
| D55 | RESOLVED | A: next 96 completed M15 candles, excluding contact candle. F: 96 full M15 intervals beginning strictly after confirmation time; no partial enclosing interval. Authorized incomplete tail is censored, not read across research boundary. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
| D56 | RESOLVED | Pre-entry contact+structural breach is unlabelable/no trade. Post-entry SL+2R same-bar unresolved order => STOP_FIRST; neither within full horizon => TIMEOUT. Keep descriptive excursions separate; coverage failures are not losses. | R, P, E; explicitly approved PRE-RESULT semantic closure. |
...
## 19. Remaining researcher decisions after Semantic Closure V1

**D20 remains OPEN:** no numerical delayed-displacement W is frozen. Do not infer it from displacement lookback or synthetic fixtures. Delayed reclaim is separately deferred, with no reclaim-window value. D19 (S/W) and D50 (A/F) remain CAMPAIGN_VARIABLE; semantic resolution is not permission to run historical data. Other previously OPEN semantics are specified above; implementation readiness is separately verified, especially F coverage/scheduling.

DEFERRED IDs: D26/D29 (IFVG), D32 (descriptive dealing-leg refinement), D36 (OTE). EXCLUDED D57: advanced management and execution costs. Other deferred concept families remain absent. If the implementation cannot safely execute an approved path it must remain fail-closed and report the implementation blocker without pretending that a fixture contract authorizes production.
